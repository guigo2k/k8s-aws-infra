...
resource "aws_subnet" "public" {
  ...
  tags                    = "${merge(map("Name", "${var.env}.${var.name}-sn-public-${data.aws_availability_zone.az.*.name_suffix[count.index]}"), var.tags, var.clusters)}"
}

resource "aws_route_table" "public" {
  vpc_id = "${var.vpc_id}"
  tags   = "${merge(map("Name", "${var.env}.${var.name}-rt-public"), var.tags, var.clusters)}"
}
...
resource "aws_subnet" "private" {
  ...
  tags                    = "${merge(map("Name", "${var.env}.${var.name}-sn-private-${data.aws_availability_zone.az.*.name_suffix[count.index]}"), var.tags, var.clusters)}"
}

resource "aws_route_table" "private" {
  count  = "${length(var.availability_zones)}"
  vpc_id = "${var.vpc_id}"
  tags   = "${merge(map("Name", "${var.env}.${var.name}-rt-private-${data.aws_availability_zone.az.*.name_suffix[count.index]}"), var.tags, var.clusters)}"
}
...
resource "aws_security_group" "default" {
  ...
  vpc_id      = "${var.vpc_id}"
  tags        = "${merge(map("Name", "${var.env}.${var.name}-sg-default"), var.tags, var.clusters)}"
}
...
resource "aws_security_group" "nat" {
  ...
  vpc_id      = "${var.vpc_id}"
  tags        = "${merge(map("Name", "${var.env}.${var.name}-sg-nat"), var.tags, var.clusters)}"
  depends_on  = ["aws_nat_gateway.nat_gw"]
}
